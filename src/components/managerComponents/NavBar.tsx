...
const NavBar:React.FC=()=>{
  ...
  const companyName=useSelector((state:RootState)=>state.manager.companyName);
    ...
    <ul className="space-y-4">
      ...
      <Link to="/manager/verifier" className="block hover:bg-blue-200 rounded p-2 text-black">
       Verifier
      </Link>
      <Link to="/manager/chat" className="block hover:bg-blue-200 rounded p-2 text-black">
        Chat
      </Link>
    </ul>
